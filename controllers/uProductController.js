...
const uprofileController = async (req, res) => {
    ...
    try {
        ...
        // console.log(result);
        // const orderecord = Array.isArray(data) ? data : [data];
        // console.log(data);
        res.render('pages/user_profile', { data: result ,orderecord:data});
...
const edituserprofile = async (req, res) => {
    try {
        ...
        if (userData) {
            ...
            res.redirect('pages/user_profile', { data: "" ,orderecord:""});
        }
    }
    catch (error) {
        console.log(error)
...
const updateuserprofile = async (req, res) => {
    try {
        ...
        res.render('pages/user_profile', { data: UserData ,orderecord:""});
    }
    catch (error) {
        console.log(error)
    }
...
export const authenticateJWT = (request, response, next) => {
    ...
    jwt.verify(token, SECRET_KEY, (err, payload) => {
        if (err)
            response.render('pages/user_login', { msg: "Token is not valid " });
...
export const confirmPasswordController = async (request, response) => {
    ...
    try {
        ...
        const updatedData = await UserRegistration.findByIdAndUpdate({ _id: user_id },
             { $set: { password: hashedPassword, confirmpassword: confirmpassword, token: '' } });
    ...
    try {
        ...
        if (result) { 
            ...
            if(cartId){
                ...
                console.log(result);
                return res.render('pages/generate_bill', {orders:placeorder,data:result,cdata:cdata});
            }
            else{
...
export const upayController = async (req, res) => {
    ...
    try {
        ...
        const uresult1 = await UserCart.find({ $and: [{ customer_id: cdata._id }, { order: "Yes" }] });
        console.log("cart data ", uresult1);
        var cartarray = [];
        var total = 0;
        for (var i = 0; i < uresult1.length; i++) {
            cartarray[i] = {};
            cartarray[i].name = uresult1[i].product_name;
            cartarray[i].sku = uresult1[i]._id;
            cartarray[i].price = uresult1[i].per_product_price;
            cartarray[i].currency = "USD"
            cartarray[i].quantity = uresult1[i].total_product;
            cartarray[i].orders = uresult1[i].order;
            total += uresult1[i].product_price;
...
export const showOrdersController = async (req,res)=>{
    console.log("show order Controller");
    try {
...
export const cancelOrder=async(req,res,next)=>{
    ...
    console.log(id);
    try{
       const cancel= await  UserCart.findByIdAndUpdate({_id:id},
            {$set:{order:'cancel'}
        });
        console.log("inside cancel order",cancel);
        next();
    }catch(err){
